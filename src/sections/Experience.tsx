  ...
  return (
    <section id="experience" className="border-t border-white/10 py-20 md:py-28">
      <div className="section-shell">
        ...
        <ol className="relative space-y-8 border-l border-white/10 pl-6 md:pl-8">
          {experience.map((role) => (
            <li key={role.id} className="relative">
              <span className="absolute -left-[31px] top-1.5 h-3 w-3 rounded-full bg-accent md:-left-[39px]" />
              <Reveal>
                <article className="border border-white/10 bg-ink-800/60 p-6 md:p-8">
                  <p className="eyebrow">{role.company}</p>
                  <h3 className="mt-2 text-2xl font-semibold text-white">{role.position}</h3>
                  <p className="mt-2 text-sm text-mist-400">
                    {isPlaceholderValue(role.duration) ? (
                      <span className="placeholder-chip">{role.duration}</span>
                    ) : (
                      role.duration
                    )}
                  </p>
